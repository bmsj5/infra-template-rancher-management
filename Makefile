...
.PHONY: help init plan apply destroy export-outputs setup-env deploy-traefik deploy-sealed-secrets deploy-cert-manager deploy-rancher deploy-all
...
deploy-sealed-secrets: ## Deploy Sealed Secrets controller
	@echo "Deploying Sealed Secrets..."
	@cd $(TF_DIR) && \
		KUBECONFIG="$$($(TF_BINARY) output -raw kubeconfig_path)" \
		ansible-playbook -i localhost, $(ANSIBLE_PLAYBOOKS_DIR)/deploy-sealed-secrets.yml
...
deploy-traefik: ## Deploy Traefik ingress controller
	...
	@cd $(TF_DIR) && \
		...
		ansible-playbook -i localhost, $(ANSIBLE_PLAYBOOKS_DIR)/deploy-traefik.yml
...
deploy-cert-manager: ## Deploy cert-manager
	...
	@cd $(TF_DIR) && \
		...
		ansible-playbook -i localhost, $(ANSIBLE_PLAYBOOKS_DIR)/deploy-cert-manager.yml
...
deploy-rancher: ## Deploy Rancher server
	...
	@cd $(TF_DIR) && \
		...
		ansible-playbook -i localhost, $(ANSIBLE_PLAYBOOKS_DIR)/deploy-rancher.yml

deploy-all: deploy-sealed-secrets deploy-traefik deploy-cert-manager deploy-rancher ## Deploy all services in order
